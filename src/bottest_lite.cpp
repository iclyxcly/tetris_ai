...
int main(void)
{
    ...
    while (true)
    {
        ...
        TetrisPathManager path(active, config, map, mino_manager.get_move_cache()[(TetrisMinoType)active.type], mino_manager.get()[(TetrisMinoType)active.type]);
        auto result = path.run_lite(map, status, active.type, config);
        result.path += "V";
        TetrisInstructor instructor(map, mino_manager.get_move_cache()[(TetrisMinoType)active.type], mino_manager.get()[(TetrisMinoType)active.type]);
